...
An Integrated Population Model(IPM), jointly analyzes data on population size and demographic parameters. Data collected in the field inform the demographic parameters in the model and unifying the analyses of multiple sources of data. The outputs from survival models and sightability models run in the PopR website and harvest data from the IDFG database are used as data in the IDFG IPM. The IPM analyzes populations at the [<b>DAU</b>](#gl-DAU){style="color: #086e04;"} level.
...
The ratios window allows us to break down the demographics that make up our study population, either by [<b>age</b>](#gl-ageratio){style="color: #086e04;"}.  or [<b>sex</b>](#gl-sexratio){style="color: #086e04;"}. 
...
The Misc window contains many rates that are useful for understanding a population. Growth rate tracks how a population grows as a proportion of the population of the year before. For example, if a population of 100 animals grows by 50 in one year, the growth rate would be 0.5. MSY allows you to select which vital rate will be displayed on your y-axis. Lambda measures growth year by year, recruitment measures population growth, but specifically how many juveniles became adults in a population and how many adults immigrate into a population. Harvest and Harvest rate are often confused, but mean slightly different things. Harvest is the number of animals whose cause of death would be hunter-related, where harvest rate is the proportion of the population that was harvested. Effort measures how hunters put pressure on a population, and is either measured by the number of hunters on the landscape or how many cumulative days that hunters spent in the field.
...
An IPM creates several outputs (Harvest, Abundance, Harvest Rate, Survival, and Ratios) and for each one it separately tests all possible values and the likelihood of each one, selecting the output with the highest probability of being true. Each value can have an immense number of possible options, so it is important to make sure that your model tests enough of them to be confident. An Rhat value of 1.1 or greater means that the model is content with the number of tests it ran, and an Rhat value of 1.1 or less means that more tests need to be run and the [<b>MCMC iterations</b>](#gl-mcmciter){style="color: #086e04;"} number needs to be increased. The closer Rhat comes to 0, the less confident the model is.
...
Model Definition and Model Fitting save your model inputs for future use by you or your peers, and report the options you selected from the top left window of the [Setup](#ipm-setup) tab. The [<b>species</b>](#gl-species){style="color: #086e04;"}, [<b>DAU</b>](#gl-DAU){style="color: #086e04;"}, and database inputs are all in the Overview tab and the years range is in the Harvest tab. The Structure section described the options in the corresponding tab in Setup, and the settings under Model Fitting can be found in the Run tab.

Diagnostics gives a sort of model review, where it lets you know the likelihood that your model had enough time to find a helpful solution. An IPM creates several outputs (Harvest, Abundance, Harvest Rate, Survival, and Ratios) and for each one it separately tests all possible values and the likelihood of each one, selecting the output with the highest probability of being true. Each value can have an immense number of possible options, so it is important to make sure that your model tests enough of them to be confident. An Rhat value of 1.1 or greater means that the model is content with the number of tests it ran, and an Rhat value of 1.1 or less means that more tests need to be run and the [<b>MCMC iterations</b>](#gl-mcmciter){style="color: #086e04;"} number needs to be increased. The closer Rhat comes to 0, the less confident the model is.

The Harvest, [<b>Abundance</b>](#gl-abundance){style="color: #086e04;"}, Adult Harvest Rate, [<b>Survival</b>](#gl-surv){style="color: #086e04;"}, and Ratios sections correspond to information in the [Plots](#ipm-plot) section. More specifically, Abundance, Survival, and Ratios are summaries of the plot windows of the same name. Abundance simply unifies the different [<b>age</b>](#gl-ageratio){style="color: #086e04;"} and [<b>sex</b>](#gl-sexratio){style="color: #086e04;"} groups into one major output, while Survival and Ratios display the different groups as multiple visualizations side by side. 
