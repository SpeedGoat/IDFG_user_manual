...
# Sightability {#sight}
  Sightability models estimate abundance in an area by combining the number of animals that we HAVE seen in aerial surveys with the number of animals that we HAVE NOT seen, which is estimated using detection probability models. These detection models consider ground vegetation, the animal’s activity, snow cover, and the size of the group of animals to determine the likelihood that an animal is seen. 
...
### Reviewing Outputs {#sight-look}
  ...
  Now that your data has been prepared you can go to the Tables tab, where you can view the output of your model. 
  ...
  To get a more detailed report of your model that includes the information in the Tables tab while keeping track of model settings, click the Build Report tab under Sightability. A prompt will appear asking you to download the report as an .html file. 
  
   <center>![](./www/sight_walk7.png)</center>
  
  <center>![](./www/ipm_walk9.png)</center>
  
  If you would like to upload the results of your model to an IPM database, click on the Admin tab. 
...
<center>![](./www/sight_report1.png)</center>
  ...
  The sections Settings and Input data have to do with the information you supplied, while the Results section has to do with the information that the model generates.
  
  The Settings section corresponds to the options you selected in the User Input window of the [Setup](#sight-setup) section.
  
  Input data summarizes the surveys that the model used to generate estimates. This information can be found in the preview window on the right side of the setup tab, and describes how you grouped your subunits when you flew sightability surveys and what proportion of each group was actually surveyed. Count data is the raw data from each individual flight.
  
  The Results section displays the outputs of the model, and each subsection (Estimates, Sampling Design, Detection Proababilit, and Covariates) corresponds to the similarly named tab from the [Tables](#sight-table) section. 
  
  The Estimates section gives the model estimates for numbers of animals, seperated into [<b>age</b>](#gl-ageratio){style="color: #086e04;"} and [<b>sex</b>](#gl-sexratio){style="color: #086e04;"} classes, as well as ratios comparing the number of males per female (MF Ratio) and the number young per female (YF Ratio.) on average in the study area.
  
  The Sampling Design section gives a summary of which [<b>DAU</b>](#gl-dau){style="color: #086e04;"} you worked in, as well as the number of that DAU's subunits that you sampled.
  
  Detection Probability's summary section organizes every group sighting that you had by the likelihood that you were going to see them. Our model determines your probability of seeing an animal based on the number of animals in the group, the type of vegatation they were spotted in, what the animals were doing when they were spotted, and the amount of snowcover at the time. 
  
  Each row in the Details section corresponds with a group of animals spotted from a plane and breaks them down by the GMU and subunit where the fighting took place, as well as the number of animals in the group and which demogroahic category they belonged to. For reference: YOY = Deer less than 1, YrlgMale = Male deer older than 1 but younger than 2, TwoYrMale = Male deer older than 2, AdFemale = Female deer older than 1, UnClass = Surveyor was unable to tell which group the deer belonged to.
  
  The covariates section gives you a glimpse into how our models work by showing the covariate options, their [<b>model values</b>](#gl-modelv){style="color: #086e04;"}, and their [<b>beta</b>](#gl-beta){style="color: #086e04;"}. This is not a table of outputs, but rather an insight as to how our model interprets the data you put into it. For example, while it would be tempting to spend precious time on a survey flight determining a specific snowcover percentage, a look at the covariates tab will tell you that snowcover is actually a categorical variable. Specifically, snowcover has three categories (1%-20%, 21%-79%, 80%-100%), which means that finding an exact percentage is a waste of time, since all values within the discrete categories will be treated the same. The reason you cannot see the 80-100% category on the covariates page is because of how our model interprets snowfall. Instead of three separate categories, all categories are in terms of the 80%-100% one. Our model does not think of the 1%-20% and 21-79% categories as independent, but as (80%-100% category) - (some value). 80%-100% is the default category, and default values are not displayed in the table.
