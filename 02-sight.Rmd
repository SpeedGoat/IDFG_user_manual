...
The next step is to enter the metadata for your survey. The metadata entry tool exists to define the survey and prevents the user from having to re-enter data that remains constant for a survey period. For example, the total number of subunits available to sample in each stratum remains constant over an entire survey period (generally a single year).
